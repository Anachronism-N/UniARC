...
from xares_llm.config_paths import AVAILABLE_TRAINING_CONFIGS, AVAILABLE_EVALUATION_CONFIGS
...
class XaresLLMTrainConfig:
    ...
    def from_file(
        ...
        with open(config_file) as con_read:
            yaml_config = yaml.safe_load(con_read)
...
class XaresLLMTask:
    def __init__(self, train_config: XaresLLMTrainConfig):
        ...
        decoder_name = Path(self.train_config.decoder_model_name).name
        self.output_dir = Path(train_config.output_dir) / train_config.config_name / decoder_name / model_name
        ...
        if self.tokenizer.pad_token is None:
            ...
            logger.info(f"Pad token was None, set to EOS token: {self.tokenizer.pad_token}")
        ...
        model_init_function = lambda: XaresLLMModel(
            config=XaresLLMModelConfig(
                ...
                projector_type=self.train_config.projector_type,
                lora_enabled=self.train_config.lora_enabled,
    ...
    def train_mlp(self) -> XaresLLMModel:
        ...
        if self.model is not None:
             # If model was loaded in __init__, it means a checkpoint exists.
             # We should try to resume training to ensure completion.
