...
def warn_and_continue(exn):
    if isinstance(exn, wds.autodecode.DecodingError):
        ...
        cause = exn.__cause__ if exn.__cause__ else exn
        logger.warning(f"Warning, trouble decoding Tar: {exn.url} File: {exn.key} Key: {exn.k} Cause: {repr(cause)}")
    else:
