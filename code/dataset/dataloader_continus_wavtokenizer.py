...
def collate_fn(data_list):
    """
    修改后的 collate_fn，用于将音频数据填充并打包成批处理张量。
    """
    # 1. 将数据列表 (list of tuples) 解压成独立的列表
    # item[0] 是音频 (numpy array), item[1] 是文本 (string)
    audios_np = [item[0] for item in data_list]
    texts = [item[1] for item in data_list]

    # 2. 将 numpy 音频数组列表转换为 PyTorch 张量列表
    #    同时，记录下每个音频在填充前的原始长度，这在模型中创建attention mask时至关重要
    audio_tensors = [torch.from_numpy(audio.astype(np.float32)) for audio in audios_np]
    original_lengths = torch.tensor([len(audio) for audio in audio_tensors], dtype=torch.long)

    # 3. 使用 pad_sequence 对张量列表进行填充
    #    - batch_first=True 表示返回的张量形状为 [batch_size, max_length]
    #    - padding_value=0.0 用0来填充
    padded_audios = pad_sequence(audio_tensors, batch_first=True, padding_value=0.0)

    # 4. 返回批处理好的数据
    #    现在，模型将接收一个单一的音频张量，而不是一个列表
    return padded_audios, texts, original_lengths
