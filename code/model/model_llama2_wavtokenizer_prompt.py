...
class IS(pl.LightningModule):
    ...
    def forward(self,inputs):
        ...
        with torch.no_grad():   
            for w in audio:
                w = torch.tensor(w, dtype=torch.float32) / 32768.0
                w = convert_audio(w.unsqueeze(0), 16000, 24000, 1).to(self.device)
                
                bandwidth_id = torch.tensor([0]).to(self.device)
                feature, _ = self.wavtokenizer.encode_infer(w, bandwidth_id=bandwidth_id)

                feature = feature.squeeze(0).transpose(0,1)
                # print("feature:",feature.shape)
                features.append(feature)
                print("feature:",feature.shape,feature)
                feat_lengths.append(feature.size(0))
        ...
        padded_features = torch.zeros(batchsize, max_feat_len, feat_dim, device=self.device)
        ...
        # print("features:",padded_features.shape,padded_features)

        # audio_inputs=features.contiguous()
        print("features:",features)

        # print(audio_inputs.shape)
        torch.cuda.empty_cache()
        audio_inputs=self.audio_embedding_last_Linear(padded_features)
        # print("audio_input:",audio_inputs)
        audio_lengths = torch.tensor(feat_lengths, device=self.device)
    ...
    def inference(self,inputs):
        ...
        print("padded_feature:",padded_features.shape)
        audio_inputs=self.audio_embedding_last_Linear(padded_features)
