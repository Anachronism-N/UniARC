...
# Source: Anachronism-N/UniARC, code/model/model_llama2_wavlm_prompt.py
...
class IS(pl.LightningModule):
    ...
    def __init__(self, hubert_ckpt_path: str=None, wavlm_ckpt_path: str=None, llama_ckpt_path: str=None, task_prompt: str='Identify the text corresponding to the speech: ', learning_rate: float=1e-05):
    ...
    def forward(self, inputs):
        ...
        batchsize = len(audio)
        audio_len = self.audio_model._get_feat_extract_output_lengths(
            torch.tensor([len(item) for item in audio])
        ).tolist()
